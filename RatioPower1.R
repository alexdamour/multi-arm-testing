...
ratio_power = function(Y, r1, rk, N, K, compute_pcombine, compute_power, knum = 1, ktype = "homo", ptype = "min", power_rep = N, n){
  ...
  p_cv = quantile(compute_pcombine(Y_null, sigk, N, K, ptype, n), 0.05)
  # get the sparse effect 
  for (j in 1:knum){
    power[j] = compute_power(Y, sigk, N, K, p_cv, ptype, R, n)
  }
...
power_ratio_test = function(Y, N, K, n_r1, n_rk, ktype = "homo", knum = 1, ratio_type, sig_type, ratio_rep = 50, step_r1 = 0.1, step_rk = 0.1, n){
  ...
  if (ktype == "homo" | ktype == "sig-hetero"){
    for (i in 1:n_r1){ # Row
      ...
      for (j in 1:n_rk){ # Column
        rk_j = j * step_rk
        rk_item[j] = rk_j
        power_matrix[i,j] = ratio_power(Y, r1_i, rk_j, N, K, compute_pcombine, compute_power, ktype = ktype, n) 
      }
    }
  }else{
    if (ktype == "rk-hetero"){
      for (i in 1:n_r1){
        ...
        for (j in 1:n_rk){ # Column
          ...
          for (e in 1:ratio_rep){ # repeated experiments
            ...
            power_exp[e] = ratio_power(Y, r1_i, rk_exp, N, K, compute_pcombine, compute_power, ktype = ktype, knum = knum, n)
          }
